...
public static class fixedSudoku
{
    ...
    /*public static List<string> easy = ReadXML.parseXmlFile("", "");
    public static List<string> medium = ReadXML.parseXmlFile("", "");
    public static List<string> hard = ReadXML.parseXmlFile("", "");*/

    // Taken from ReadXML class but changed parameters.
    ...
    public static List<string> easy = parseXmlFile(sudokuEasy, "//ArrayOfLevel/Level/ArenaHorizontalLines/Line");
    public static List<string> medium = parseXmlFile(sudokuMedium, "//ArrayOfLevel/Level/ArenaHorizontalLines/Line");
    public static List<string> hard = parseXmlFile(sudokuHard, "//ArrayOfLevel/Level/ArenaHorizontalLines/Line");
...
public class SudokuControl : MonoBehaviour
{
    ...
    void Start ()
    {
        ...
        */
        ...
        fixedSudoku.xmlDoc.LoadXml(fixedSudoku.sudokuEasy.text);


        if (fixedSudoku.xmlDoc != null)
        {
            Debug.Log("Happy " + fixedSudoku.xmlDoc);
        }
        ...
        int squareIndex = 0;
        if (all_number_squares.Length > 0)
        {
            for (int i = 0; i < 9; i++)
            {
                for (int j = 0; j < 9; j++)
